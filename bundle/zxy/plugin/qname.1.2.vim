...
if !exists("g:qname_hotkey") || g:qname_hotkey == ""
	...
	"let g:qname_hotkey = "zx"
	let g:qname_hotkey = "<A-a>"
endif
"if !exists("g:qname_hotkey2") || g:qname_hotkey2 == ""
	"let g:qname_hotkey2 = "<A-z>"
"endif
exe "nmap" g:qname_hotkey ":cal QNameInit(1)<cr>:~"
"exe "nmap" g:qname_hotkey2 ":cal QNameInit(1)<cr>:~"
...
function! QNameRun()
	try
		...
		if _key ==? "\<A-F>" || _key ==? "\<A-S>" || _key ==# "\<ESC>" || _key ==# "\<CR>" || _key ==# "\<S-CR>" || _key ==# "\<A-CR>"
			let _sel = s:colPrinter.sel
			if _key != "\<ESC>" && _sel < len(s:n) && _sel >= 0
				if _key ==? "\<A-S>" || _key ==# "\<S-CR>" || _key ==# "\<A-CR>"
					"let n = inputdialog("value for key", char2nr(_key))
					:split
				endif
				call s:swb(str2nr(matchstr(s:s[_sel], '<\zs\d\+\ze>')))
			endif
			call QNameInit(0)
		elseif  _key ==? "\<ScrollWheelUp>" || _key ==? "\<A-E>" || _key ==? "\<A-K>" || _key ==# "\<Up>" || _key ==# "\<S-Tab>"
			call s:colPrinter.vert(-1)
		elseif  _key ==? "\<ScrollWheelDown>" || _key ==? "\<A-D>" || _key ==? "\<A-J>" || _key ==# "\<Down>" || _key ==# "\<Tab>"
			call s:colPrinter.vert(1)
		elseif _key ==? "\<S-ScrollWheelUp>" || _key ==? "\<A-H>" || _key ==# "\<Left>"
			call s:colPrinter.horz(-1)
		elseif _key ==? "\<S-ScrollWheelDown>" || _key ==? "\<A-L>" || _key ==# "\<Right>"
			call s:colPrinter.horz(1)
